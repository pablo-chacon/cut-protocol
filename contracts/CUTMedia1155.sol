...
/// @notice CUT Release primitive (v0, Model A, cross-medium).
...
contract CUTMedia1155 is ERC1155 {
    ...
    struct Release {
        ...
        bytes32 radioRoot;      // optional: commitment to radio/discovery set (can be 0 for non-music)
    ...
    // ---------------------------------------------------------------------
    // Release creation (once)
    // ---------------------------------------------------------------------

    function createRelease(
        bytes32 sceneId,
        bytes32 mediumType,
        bytes32 radioRoot,       // allow 0 for non-music mediums
        bytes32 contentRoot,
    ...
    function mintReleaseCopy(
        ...
        if (amount == 0) revert InvalidRecipient();
        ...
        if (priceWei > 0) {
            ...
            if (fee > 0) {
                ...
                emit ProtocolFeePaid(releaseId, protocolTreasury, fee);
            }
        ...
        emit ReleaseCopyMinted(releaseId, msg.sender, to, amount, priceWei, fee);
    }
    ...
    // ---------------------------------------------------------------------
    // Radio verification (unchanged semantics, but radioRoot may be 0)
    // ---------------------------------------------------------------------
